...
Our [initial Creation of this SVD Projection](https://shainarace.github.io/docs/SVD_shows_briefs_vs_articles.html) allowed us to see that we had an issue with _briefs_ vs. _articles_ in this data, something that may have caused noise in subsequent analysis had we not been careful about the exploratory phase. 
...
[Full Page Visualization](https://shainarace.github.io/docs/svd_projection.html)
...
[Full Page Visualization](https://shainarace.github.io/docs/UMAP_noClusters.html)
...
[Full Page Visualization](https://shainarace.github.io/docs/All_clusters_noTopics_UMAPClus_wNoise.html)
...
[Full Page Visualization](https://shainarace.github.io/docs/All_clusters_noTopics_UMAPClus.html)


# Cluster Refinement

Problems with our initial clustering: \
1. Many medium-big clusters get broken up even though they seem nicely separated on the UMAP projection. \
2. The big center blob is begging to be dealt with a bit better.\ 

Three ideas for potential refinement:\
...
3. __Take all the existing clusters as single documents (i.e. paste all documents in a cluster together into single document for each cluster) -- repeat this process.__ I think this is a nice idea but I don't believe I'll have time to play with it. 
...
```{r message=F, warning=F, eval=F}
# cen_clus = hdbscan(centroids, 3) # Down to 103 Clusters...Looks Pretty Good. 
...
[Full Page Visualization](https://shainarace.github.io/docs/Centroid_Clus.html)
...
[Full Page Visualization](https://shainarace.github.io/docs/All_centroid_refined_clusters.html)


Onto the next thought for refinement: divide and conquer.

## Refinement Idea 2: Divide and Conquer

Here, we divide the data according to UMAP and recompute the SVD of that subset. We see _some_ better cluster separation than we did in the corresponding rectangle on our original umap viz, but the end result still leaves a lot of room for improvement. 


```{r message=F, warning=F, eval=F}
# Take rectangular subset on the interval x=[-0.5,3] and y=[-1.5,1]
index_subset2 = svd_ump[,1] <3 &svd_ump[,1] > -0.5 &  svd_ump[,2] <1&  svd_ump[,2] > -1.5
...
tfidf_tdm_subset = apply(tfidf_tdm_subset, 2, function(x){x/c(sqrt(t(x)%*%x))})
tfidf_tdm_subset <- as(tfidf_tdm_subset, "sparseMatrix")

# Take SVD of the subset and compute the UMAP 
svd_subset = irlba(tfidf_tdm_subset,150)
...
# Cluster
clus2=hdbscan(svd_subset_map,4)
...
saveWidget(fig2, 'docs/SubsettingUMAPforRepeatSVD.html')


```


[Full Page Visualization](https://shainarace.github.io/docs/SubsettingUMAPforRepeatSVD.html)